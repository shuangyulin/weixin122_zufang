<template>
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback" :bottom="100">
		<view class="container">
					...
					<view class="lable" :style='{"width":"170rpx","fontSize":"32rpx","color":"rgba(0, 0, 0, 1)","textAlign":"left"}'>名称：</view>
					<view class="text" :style='{"color":"rgba(0, 0, 0, 1)","textAlign":"left","fontSize":"28rpx"}'>{{detail.name}}</view>
				</view>
...
<script>
	export default {
		...
		methods: {
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussconfig/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('config', _this.detail);
